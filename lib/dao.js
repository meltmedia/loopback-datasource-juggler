  ...
  var allCb = function(err, data) {
    ...
    if (!err && Array.isArray(data)) {
      ...
      function(err) {
        ...
        // This should ideally not be needed, but resolves an issue with model order getting out of sync
        if (results.length > 0 && results[0].getIdName) {
          var idName = results[0].getIdName();
          var ids = data.map(function (o) { return o[idName]; });
          results = utils.sortObjectsByIds(results[0].getIdName, ids, results, true);
        }
